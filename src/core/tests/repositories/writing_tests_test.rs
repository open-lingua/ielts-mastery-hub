...
mod insert_and_find_by_id {
    ...
    async fn it_finds_the_test_when_owner_looks_it_up() {
        ...
        let input = CreateWritingTestBuilder::default()
            .with_title("Opinion Essays")
            .build();
        let id = writing_tests::insert(&pool, &input, OWNER_ID)
            .await
            .expect("insert");
...
mod find_all {
    ...
    async fn it_excludes_other_users_drafts() {
        ...
        // Act
        let all = writing_tests::find_all(&pool, OTHER_USER_ID)
            .await
            .expect("find_all");
...
mod update {
    ...
    async fn it_updates_only_the_provided_field() {
        ...
        let id = writing_tests::insert(&pool, &input, OWNER_ID)
            .await
            .expect("insert");
        let update = UpdateWritingTestBuilder::default()
            .with_status("published")
            .build();

        // Act
        writing_tests::update(&pool, &id, OWNER_ID, &update)
            .await
            .expect("update");
        let found = writing_tests::find_by_id(&pool, &id, OWNER_ID)
            .await
            .expect("find")
            .expect("present");
    ...
    async fn it_does_not_update_a_test_owned_by_another_user() {
        ...
        // Act
        writing_tests::update(&pool, &id, OTHER_USER_ID, &update)
            .await
            .expect("update");
        let found = writing_tests::find_by_id(&pool, &id, OWNER_ID)
            .await
...
mod delete {
    ...
    async fn it_removes_the_test_when_owner_deletes_it() {
        ...
        // Act
        writing_tests::delete(&pool, &id, OWNER_ID)
            .await
            .expect("delete");
        let found = writing_tests::find_by_id(&pool, &id, OWNER_ID).await;
    ...
    async fn it_does_not_remove_a_test_owned_by_another_user() {
        ...
        // Act
        writing_tests::delete(&pool, &id, OTHER_USER_ID)
            .await
            .expect("delete");
        let found = writing_tests::find_by_id(&pool, &id, OWNER_ID).await;
