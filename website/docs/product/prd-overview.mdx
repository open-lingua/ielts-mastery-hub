...
:::warning TS-2 is not yet fully implemented
As documented in [Admin CMS Overview](../admin/overview.mdx) and [Frontend Architecture](../architecture/frontend-architecture.mdx), **no `ProtectedRoute` component exists** in the current codebase, and `/admin/*` routes are reachable by anyone regardless of `user_roles`. The `user_roles` table (`student`/`super_admin`) exists and can be populated via the User Management UI, but nothing currently reads it to gate route access. This is the clearest gap between this PRD and the shipped implementation.
:::
