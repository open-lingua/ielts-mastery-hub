...
| `claude` | `POST https://api.anthropic.com/v1/messages` | `claude-3-5-sonnet-latest` (overridable via an optional `model` credential, like `local`/`general`/`gemini`/`chatgpt`) | `x-api-key` header + `anthropic-version: 2023-06-01` |
