...
- `upload_writing_asset` (domain `writing-assets`) stores the file as `$HOME/.imh/writing-assets/<writing_task_id>.<ext>` — no per-user subfolder; the filename is the `writing_task` id (generated client-side before upload so it can be threaded through to `create_writing_tasks`/`import_writing_test`). Re-uploading for the same task id overwrites the file.
